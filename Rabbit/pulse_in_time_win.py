...
class PulseInTimeWin(QDialog):
    ...
    def refreshplot_fn(self) -> None:
        ...
        try:
            ...
            # DEBUG
            # self.ax.plot(cts.energy_vect, cts.rabbit_mat[0, :])
    ...
    # We suppose that is has the form cts.peak_phase = [phase_SB_14, phase_SB_16, …]
    def temporal_profile_fn(self, time):

        harmonics_spectral_phases = []
        # DEBUG
        # DEBUG_phases = [1, 2, 3, 4, 8]
        ...
        # print(harmonics_energies)
        print(harmonics_amplitudes)

        # fields_sum is the sum of the fields of each harmonic
        fields_sum = 0.0

        # fields_sum = harmonics_amplitudes[0] * np.exp(
        #     - 2 * 1j * np.pi * (cts.first_harm) * cts.cur_nu * time + harmonics_spectral_phases[0])

        for i in range(cts.bandsnb + 1):
            fields_sum += harmonics_amplitudes[i] * np.exp(- 2 * 1j * np.pi * (cts.first_harm + 2*i) * cts.cur_nu * time + harmonics_spectral_phases[i])

        # Return the intensity of the calculated field
        return np.abs(fields_sum)**2
